...
const CACHE = "eyevos-v5";   // bump on every shell-changing deploy so activate() purges the stale cache
const SHELL = ["/", "/index.html", "/app-live.html", "/leagues.js", "/privacy.html", "/manifest.webmanifest", "/icon.svg"];
...
self.addEventListener("fetch", (e) => {
  ...
  if (req.mode === "navigate" || req.destination === "document") {
    e.respondWith(
      fetch(req)
        .then((r) => { if (r && r.ok && r.type === "basic") { const cp = r.clone(); caches.open(CACHE).then((c) => c.put(req, cp)); } return r; })  // never cache a 404/500/redirect as the offline shell
